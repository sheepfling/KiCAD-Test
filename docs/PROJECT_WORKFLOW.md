...
- Treat a library, rule, project-format, or mechanical-interface change as reviewable engineering work, not incidental cleanup.
- Run ERC and DRC, inspect every warning/exclusion, and keep generated review evidence outside tracked source.
...
Stop when a project opens with a new KiCad build, a conversion warning, or unexplained source rewrite. Preserve the worktree and report the branch, commit, KiCad version, `git status`, and diff. Perform migration in a dedicated PR; do not combine it with board edits.
