# arduino-uno-status-led

Training pcb project — NOT FOR MANUFACTURE.

- [Project manifest](project.json): identity, toolchain, local inventory and shared dependencies.
- [Design notes](docs/design.md): purpose and interface assumptions.
- [Native project](kicad/arduino-uno-status-led.kicad_pro): open with the exact selected KiCad version.
- [Test contract](tests/contract.json): independent native expectations.

From the repository root, run `python -B -m tools.ci --project arduino-uno-status-led`.
Optional Python tests in this folder run automatically. Native checks use
`python -B -m tools.ci --kicad --project arduino-uno-status-led --output examples/projects/arduino-uno-status-led/build/review-001`.
Use a fresh output path each attempt. Keep generated evidence in ignored `build/`.
