...
from tools.hwrepo.repository import (
    cad_dependencies,
    check_repository,
    ephemeral,
    unmanaged_artifact,
...
class ProductTests(unittest.TestCase):
    ...
    def test_machine_local_cad_paths_and_unknown_variables(self):
        ...
        for value in ("C:/parts/private.pretty", "/home/parts/local.pretty", "${MY_LIBRARY}/part.pretty"):
            with self.subTest(value=value):
                ...
                self.assertTrue(cad_dependencies(self.temp, path, self.temp, "10"))
        path.write_text('(uri "${KICAD10_FOOTPRINT_DIR}/Connector.pretty")', encoding="utf-8")
        self.assertEqual(cad_dependencies(self.temp, path, self.temp, "10"), [])
    ...
    def test_repository_gate_rejects_force_added_unmanaged_artifact(self):
        ...
        with patch("tools.hwrepo.repository.subprocess.run", return_value=completed):
