...
def adopt(root: Path, project_id: str) -> TemplateAdoptReport:
    ...
    return TemplateAdoptReport(
        ...
        issues=() if passed else ("Portable acceptance failed; run tools.ci for the detailed report.",),
        next_actions=(
        ...
        ) if passed else ("Run python -B -m tools.ci and resolve its reported findings.",),
    )
